    ...
    return (
        <div className="min-h-screen safe-mobile-height flex items-center justify-center p-3 sm:p-4">
            ...
            <motion.div
                ...
                className="bg-white/80 backdrop-blur-md p-5 sm:p-8 rounded-3xl shadow-xl w-full max-w-lg relative z-10 border-4 border-primary-light"
            >
                <div className="text-center mb-6 sm:mb-8">
                    <div className="inline-block p-4 bg-primary-light rounded-full mb-4 text-primary-dark">
                        <UserPlus size={32} />
                    </div>
                    <h1 className="text-2xl sm:text-3xl font-bold text-primary-dark">Join Our Learning Family!</h1>
                    <p className="text-sm sm:text-base text-gray-600 mt-2 font-medium">Create your profile to start your adventure.</p>
                </div>

                <form onSubmit={handleSignup} className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                    <div className="md:col-span-2">
                        <label className="block text-gray-700 font-semibold mb-1">Your Full Name</label>
                        <input
                            type="text"
                            className="w-full px-4 py-3.5 rounded-2xl border-2 border-gray-100 focus:border-primary focus:outline-none text-base"
                    ...
                    <div className="md:col-span-2 space-y-4 pt-3">
                        {error && <p className="text-red-500 bg-red-50 p-3 rounded-xl text-sm font-medium text-center">{error}</p>}
                        {success && <p className="text-green-600 bg-green-50 p-3 rounded-xl text-base font-bold text-center flex items-center justify-center gap-2"><Sparkles className="animate-pulse" /> {success}</p>}
