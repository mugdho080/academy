...
try {
    ...
    $items = $service->getAchievementsForUser($userId);
    echo json_encode([
        'items' => $items,
        'unlocked_count' => count(array_filter($items, static fn($a) => !empty($a['is_unlocked'])))
    ]);
} catch (\Throwable $e) {
    http_response_code(500);
    echo json_encode(['error' => 'Failed to fetch achievements', 'details' => $e->getMessage()]);
}
